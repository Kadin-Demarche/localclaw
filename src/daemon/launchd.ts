...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.LOCALCLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.LOCALCLAW_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.LOCALCLAW_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.LOCALCLAW_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.LOCALCLAW_PROFILE)) {
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.LOCALCLAW_PROFILE,
      version: environment?.LOCALCLAW_SERVICE_VERSION ?? env.LOCALCLAW_SERVICE_VERSION,
    });
