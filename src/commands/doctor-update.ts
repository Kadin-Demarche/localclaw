...
async function detectLocalClawGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.LOCALCLAW_UPDATE_IN_PROGRESS);
  ...
  const git = await detectLocalClawGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update LocalClaw from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("localclaw update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
