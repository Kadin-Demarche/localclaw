...
export async function updateConfig(
  mutator: (cfg: LocalClawConfig) => LocalClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: LocalClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LocalClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("localclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
