...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveLocalClawAgentDir: vi.fn().mockReturnValue("/tmp/localclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/localclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/localclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveLocalClawAgentDir: mocks.resolveLocalClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveLocalClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/localclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/localclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/localclaw-agent-custom");
