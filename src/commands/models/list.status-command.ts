...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveLocalClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("localclaw models auth setup-token")}\` or \`${formatCliCommand("localclaw configure")}\`.`
          : `Run \`${formatCliCommand("localclaw configure")}\` or set an API key env var.`;
