...
beforeAll(() => {
  previousProfile = process.env.LOCALCLAW_PROFILE;
  process.env.LOCALCLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.LOCALCLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/localclaw-root.js", () => ({
  resolveLocalClawPackageRoot: vi.fn().mockResolvedValue("/tmp/localclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/localclaw/pnpm-lock.yaml",
      markerPath: "/tmp/localclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("LocalClaw status"))).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.LOCALCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
