...
import type { LocalClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: LocalClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("localclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("localclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("localclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("localclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("localclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
