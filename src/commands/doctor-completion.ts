...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(localclaw completion ...)` */
 ...
 */
export async function ensureCompletionCacheExists(binName = "localclaw"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
