...
function resolveMode(cfg: LocalClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveLocalClawPackageRoot({
  ...
  let cfg: LocalClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("localclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("localclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("localclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("localclaw doctor --fix")}" to apply changes.`);
  }
