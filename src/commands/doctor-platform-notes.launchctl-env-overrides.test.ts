...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "LOCALCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("LOCALCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv LOCALCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("LOCALCLAW_GATEWAY_PASSWORD");
  });
