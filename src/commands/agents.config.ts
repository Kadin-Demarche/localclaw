...
export function listAgentEntries(cfg: LocalClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: LocalClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: LocalClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: LocalClawConfig): AgentSummary[] {
