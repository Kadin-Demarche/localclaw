...
const SEARCH_TOOL = "https://docs.localclaw.ai/mcp.SearchLocalClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.localclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('localclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.localclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('localclaw docs "your query"')}`);
    }
    return;
  }
