...
const requireAgentDir = () => {
  const agentDir = process.env.LOCALCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("LOCALCLAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.LOCALCLAW_STATE_DIR;
  const previousAgentDir = process.env.LOCALCLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.LOCALCLAW_AGENT_DIR;
    } else {
      process.env.LOCALCLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.LOCALCLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under LOCALCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to LOCALCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-minimax-"));
    process.env.LOCALCLAW_STATE_DIR = tempStateDir;
    process.env.LOCALCLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.LOCALCLAW_AGENT_DIR;
