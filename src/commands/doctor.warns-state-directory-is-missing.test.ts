...
beforeEach(() => {
  ...
  resolveLocalClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.LOCALCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.LOCALCLAW_UPDATE_IN_PROGRESS;
  process.env.LOCALCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "localclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.LOCALCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.LOCALCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.LOCALCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveLocalClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadLocalClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/localclaw.json",
    createConfigIO,
...
vi.mock("../infra/localclaw-root.js", () => ({
  resolveLocalClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "localclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.LOCALCLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when LOCALCLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/localclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.LOCALCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
