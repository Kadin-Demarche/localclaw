...
import { discoverLocalClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: LocalClawPluginDefinition;
  register?: LocalClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as LocalClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as LocalClawPluginDefinition;
...
export function loadLocalClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverLocalClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "localclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: LocalClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as LocalClawPluginModule;
    } catch (err) {
