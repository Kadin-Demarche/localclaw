...
import { loadLocalClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `localclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "localclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadLocalClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadLocalClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadLocalClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
