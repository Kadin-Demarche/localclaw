...
function setWhatsAppDmPolicy(cfg: LocalClawConfig, dmPolicy: DmPolicy): LocalClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: LocalClawConfig, allowFrom?: string[]): LocalClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: LocalClawConfig, selfChatMode: boolean): LocalClawConfig {
...
async function detectWhatsAppLinked(cfg: LocalClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so LocalClaw can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for LocalClaw" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("localclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
