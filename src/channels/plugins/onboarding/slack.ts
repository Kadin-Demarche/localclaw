...
function setSlackDmPolicy(cfg: LocalClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "LocalClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for LocalClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: LocalClawConfig, allowFrom: string[]): LocalClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
