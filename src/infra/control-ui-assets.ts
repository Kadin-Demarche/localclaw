...
import { resolveLocalClawPackageRoot, resolveLocalClawPackageRootSync } from "./localclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveLocalClawPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "localclaw" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveLocalClawPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // localclaw.mjs or dist/<bundle>.js
