...
function candidateBinDirs(opts: EnsureLocalClawPathOpts): string[] {
  ...
  // Bundled macOS app: `localclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "localclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/localclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "localclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `localclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureLocalClawCliOnPath(opts: EnsureLocalClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.LOCALCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.LOCALCLAW_PATH_BOOTSTRAPPED = "1";
