...
export function triggerLocalClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.LOCALCLAW_SYSTEMD_UNIT,
        process.env.LOCALCLAW_PROFILE,
      );
  ...
  const label =
    process.env.LOCALCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LOCALCLAW_PROFILE);
