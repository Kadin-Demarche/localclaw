...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "localclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("localclaw doctor", doctorArgv, gitRoot, { LOCALCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
