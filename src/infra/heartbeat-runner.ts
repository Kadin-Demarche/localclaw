...
function resolveActiveHoursTimezone(cfg: LocalClawConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: LocalClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: LocalClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: LocalClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: LocalClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: LocalClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: LocalClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: LocalClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
