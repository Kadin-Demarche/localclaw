...
type WebSearchConfig = NonNullable<LocalClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LocalClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("localclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.localclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://localclaw.ai",
      "X-Title": "LocalClaw Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
