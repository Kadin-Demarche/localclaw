...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-embedded-agent-"));
...
const ensureModels = (cfg: LocalClawConfig) => ensureLocalClawModelsJson(cfg, agentDir) as unknown;
