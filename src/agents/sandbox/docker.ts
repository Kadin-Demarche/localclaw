...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "localclaw.sandbox=1");
  args.push("--label", `localclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `localclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `localclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("localclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`localclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`localclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("localclaw sandbox recreate --all");
}
