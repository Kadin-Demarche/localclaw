...
const DIRECT_ENABLED = Boolean(process.env.LOCALCLAW_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.LOCALCLAW_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set LOCALCLAW_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.LOCALCLAW_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.LOCALCLAW_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.LOCALCLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
