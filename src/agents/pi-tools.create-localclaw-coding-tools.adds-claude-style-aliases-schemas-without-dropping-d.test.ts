...
const defaultTools = createLocalClawCodingTools();

describe("createLocalClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from localclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "localclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "localclaw-workspace"),
    ...
    const tools = createLocalClawCodingTools({ sandbox });
