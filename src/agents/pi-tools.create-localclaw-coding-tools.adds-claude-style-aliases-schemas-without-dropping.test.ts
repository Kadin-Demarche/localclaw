...
import { __testing, createLocalClawCodingTools } from "./pi-tools.js";
...
describe("createLocalClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createLocalClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createLocalClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "localclaw-outside.txt");
