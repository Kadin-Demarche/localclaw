...
import { createLocalClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("localclaw-ws-", async (workspaceDir) => {
      await withTempDir("localclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createLocalClawCodingTools({ workspaceDir });
    ...
    await withTempDir("localclaw-ws-", async (workspaceDir) => {
      await withTempDir("localclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello localclaw");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("localclaw-ws-", async (workspaceDir) => {
      const tools = createLocalClawCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("localclaw-ws-", async (workspaceDir) => {
      await withTempDir("localclaw-override-", async (overrideDir) => {
        const tools = createLocalClawCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("localclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("localclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "localclaw-sbx-test",
        ...
        const tools = createLocalClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
