...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[localclaw] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    // See: https://github.com/localclaw/localclaw/issues/4597
