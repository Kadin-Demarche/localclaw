...
/**
 * Cleanup suspended LocalClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: LocalClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-cli-images-"));
  const paths: string[] = [];
