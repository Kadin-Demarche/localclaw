...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "LOCALCLAW_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.LOCALCLAW_STATE_DIR = path.join(home, ".localclaw");
    ...
    await fs.mkdir(path.join(home, ".localclaw"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.localclaw${path.sep}media`);
