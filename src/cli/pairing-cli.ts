...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.localclaw.ai/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!opts.channel && !code) {
        throw new Error(
          `Usage: ${formatCliCommand("localclaw pairing approve <channel> <code>")} (or: ${formatCliCommand("localclaw pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("localclaw pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
