...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "localclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      LOCALCLAW_NO_RESPAWN: "1",
      LOCALCLAW_STATE_DIR: stateDir,
      LOCALCLAW_SKIP_CHANNELS: "1",
      LOCALCLAW_SKIP_GMAIL_WATCHER: "1",
      LOCALCLAW_SKIP_CRON: "1",
      LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      LOCALCLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "localclaw-entry-bootstrap.mjs");
