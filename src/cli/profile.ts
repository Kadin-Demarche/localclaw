...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.localclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.LOCALCLAW_PROFILE = profile;

  const stateDir = env.LOCALCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.LOCALCLAW_STATE_DIR?.trim()) {
    env.LOCALCLAW_STATE_DIR = stateDir;
  }

  if (!env.LOCALCLAW_CONFIG_PATH?.trim()) {
    env.LOCALCLAW_CONFIG_PATH = path.join(stateDir, "localclaw.json");
  }

  if (profile === "dev" && !env.LOCALCLAW_GATEWAY_PORT?.trim()) {
    env.LOCALCLAW_GATEWAY_PORT = "19001";
  }
}
