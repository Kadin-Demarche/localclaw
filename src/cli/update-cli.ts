...
import { resolveLocalClawPackageRoot } from "../infra/localclaw-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "localclaw doctor entry": "Checking doctor entrypoint",
  "localclaw doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "localclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const LOCALCLAW_REPO_URL = "https://github.com/localclaw/localclaw.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".localclaw");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("localclaw@")) {
    return trimmed.slice("localclaw@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "localclaw.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("localclaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.LOCALCLAW_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", LOCALCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `LOCALCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set LOCALCLAW_GIT_DIR to an empty folder or an localclaw checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`LOCALCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveLocalClawPackageRoot({
  ...
  defaultRuntime.log(theme.heading("LocalClaw update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating LocalClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this LocalClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("localclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("localclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g localclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g localclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.LOCALCLAW_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.LOCALCLAW_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("localclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("localclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("localclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("localclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `localclaw update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `LOCALCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set LOCALCLAW_GIT_DIR to an empty folder or an localclaw checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via LOCALCLAW_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["localclaw update", "Update a source checkout (git)"],
        ["localclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["localclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["localclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["localclaw update --no-restart", "Update without restarting the service"],
        ["localclaw update --json", "Output result as JSON"],
        ["localclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["localclaw update wizard", "Interactive update wizard"],
        ["localclaw --update", "Shorthand for localclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run localclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.localclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["localclaw update status", "Show channel + version status."],
          ["localclaw update status --json", "JSON output."],
          ["localclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.localclaw.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
