...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# LocalClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(localclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# LocalClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(localclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $LOCALCLAW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "localclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c localclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'localclaw gateway' and not 'localclaw other gateway', we need to check the sequence.
