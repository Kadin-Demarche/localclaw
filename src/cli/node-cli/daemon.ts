...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("localclaw node install"),
    formatCliCommand("localclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("localclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    LOCALCLAW_LOG_PREFIX: baseEnv.LOCALCLAW_LOG_PREFIX ?? "node",
