...
${formatHelpExamples([
  ['localclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['localclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'localclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'localclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['localclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'localclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.localclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.localclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'localclaw agents set-identity --agent main --name "LocalClaw" --emoji "🦞"',
    "Set name + emoji.",
  ],
  ["localclaw agents set-identity --agent main --avatar avatars/localclaw.png", "Set avatar path."],
  [
    "localclaw agents set-identity --workspace ~/.localclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "localclaw agents set-identity --identity-file ~/.localclaw/workspace/IDENTITY.md --agent main",
