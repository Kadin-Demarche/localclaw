...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.localclaw.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. localclaw.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.localclaw/localclaw.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by localclaw dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.localclaw/localclaw.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
