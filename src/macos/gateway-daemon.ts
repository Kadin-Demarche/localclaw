...
declare const __LOCALCLAW_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __LOCALCLAW_VERSION__ === "string" && __LOCALCLAW_VERSION__) ||
  process.env.LOCALCLAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `localclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.LOCALCLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.LOCALCLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[localclaw] Gateway daemon failed:",
