...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const localclaw = resolveProfile(resolved, "localclaw");
    expect(localclaw?.driver).toBe("localclaw");
    expect(localclaw?.cdpPort).toBe(18800);
    expect(localclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from LOCALCLAW_GATEWAY_PORT when unset", () => {
    const prev = process.env.LOCALCLAW_GATEWAY_PORT;
    process.env.LOCALCLAW_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const localclaw = resolveProfile(resolved, "localclaw");
      expect(localclaw?.cdpPort).toBe(19012);
      expect(localclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.LOCALCLAW_GATEWAY_PORT;
      } else {
        process.env.LOCALCLAW_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const localclaw = resolveProfile(resolved, "localclaw");
      expect(localclaw?.cdpPort).toBe(19022);
      expect(localclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.LOCALCLAW_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "localclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        localclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("localclaw");
  });
});
