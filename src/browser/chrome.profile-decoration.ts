...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".localclaw-profile-decorated");
}
 ...
 */
export function decorateLocalClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_LOCALCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_LOCALCLAW_BROWSER_COLOR).toUpperCase();
