...
vi.mock("./chrome.js", () => ({
  resolveLocalClawUserDataDir: vi.fn(() => "/tmp/localclaw-test/localclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveLocalClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "localclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveLocalClawUserDataDir).mockReturnValue(userDataDir);
