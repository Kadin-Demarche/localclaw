import type { BrowserConfig, BrowserProfileConfig, LocalClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_LOCALCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_LOCALCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "localclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_LOCALCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_LOCALCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an LocalClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for localclaw/localclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: LocalClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_LOCALCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "localclaw";
