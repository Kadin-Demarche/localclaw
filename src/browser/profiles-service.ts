...
import type { BrowserProfileConfig, LocalClawConfig } from "../config/config.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    const nextConfig: LocalClawConfig = {
      ...cfg,
      browser: {
        ...cfg.browser,
        profiles: {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback) {
      ...
      const userDataDir = resolveLocalClawUserDataDir(name);
