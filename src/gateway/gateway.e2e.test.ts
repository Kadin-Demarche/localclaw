...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "localclaw");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.localclaw-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".localclaw");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "localclaw.json");
      ...
      process.env.LOCALCLAW_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.LOCALCLAW_CONFIG_PATH = prev.configPath;
        process.env.LOCALCLAW_GATEWAY_TOKEN = prev.token;
        process.env.LOCALCLAW_SKIP_CHANNELS = prev.skipChannels;
        process.env.LOCALCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.LOCALCLAW_SKIP_CRON = prev.skipCron;
        process.env.LOCALCLAW_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.LOCALCLAW_STATE_DIR = prev.stateDir;
      process.env.LOCALCLAW_CONFIG_PATH = prev.configPath;
      process.env.LOCALCLAW_GATEWAY_TOKEN = prev.token;
      process.env.LOCALCLAW_SKIP_CHANNELS = prev.skipChannels;
      process.env.LOCALCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
