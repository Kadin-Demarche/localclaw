  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.LOCALCLAW_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (LOCALCLAW_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (LOCALCLAW_SKIP_CHANNELS=1 or LOCALCLAW_SKIP_PROVIDERS=1)",
        );
      } else {
