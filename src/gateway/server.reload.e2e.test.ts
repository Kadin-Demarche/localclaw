...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.LOCALCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.LOCALCLAW_SKIP_GMAIL_WATCHER;
    process.env.LOCALCLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.LOCALCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.LOCALCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.LOCALCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
