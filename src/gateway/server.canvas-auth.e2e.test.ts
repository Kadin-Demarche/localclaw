...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.LOCALCLAW_CONFIG_PATH;
  const prevDisableCache = process.env.LOCALCLAW_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "localclaw-canvas-auth-test-"));
  const configPath = path.join(dir, "localclaw.json");

  process.env.LOCALCLAW_CONFIG_PATH = configPath;
  process.env.LOCALCLAW_DISABLE_CONFIG_CACHE = "1";
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.LOCALCLAW_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.LOCALCLAW_DISABLE_CONFIG_CACHE;
    } else {
      process.env.LOCALCLAW_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
