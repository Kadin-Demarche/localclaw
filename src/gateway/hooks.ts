...
export function resolveHooksConfig(cfg: LocalClawConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-localclaw-token"] === "string"
      ? req.headers["x-localclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
