    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "localclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-localclaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-localclaw-agent-id": "beta",
            "x-localclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
