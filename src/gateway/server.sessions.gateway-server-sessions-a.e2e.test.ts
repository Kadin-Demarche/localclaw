...
beforeAll(async () => {
  previousToken = process.env.LOCALCLAW_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.LOCALCLAW_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
