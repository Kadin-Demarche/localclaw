...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.LOCALCLAW_STATE_DIR;
  previousConfigPath = process.env.LOCALCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.LOCALCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.LOCALCLAW_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.LOCALCLAW_STATE_DIR = path.join(tempHome, ".localclaw");
  delete process.env.LOCALCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.LOCALCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.LOCALCLAW_SKIP_CANVAS_HOST = "1";
  process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "localclaw-test-no-bundled-extensions")
    : "localclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "localclaw-test-"))
    : path.join(tempHome, ".localclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.LOCALCLAW_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.LOCALCLAW_CONFIG_PATH;
    } else {
      process.env.LOCALCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.LOCALCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.LOCALCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.LOCALCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.LOCALCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.LOCALCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.LOCALCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.LOCALCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.LOCALCLAW_GATEWAY_TOKEN = fallbackToken;
  }
