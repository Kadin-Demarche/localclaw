...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "localclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".localclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<LocalClawConfig> = {},
): LocalClawConfig {
  const base: LocalClawConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "localclaw"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".localclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
