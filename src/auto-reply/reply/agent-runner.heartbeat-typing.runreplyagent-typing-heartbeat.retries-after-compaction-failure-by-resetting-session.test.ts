...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("retries after compaction failure by resetting the session", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "localclaw-session-compaction-reset-"));
  ...
  it("retries after context overflow payload by resetting the session", async () => {
    const prevStateDir = process.env.LOCALCLAW_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "localclaw-session-overflow-reset-"));
    process.env.LOCALCLAW_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
  ...
  it("resets the session after role ordering payloads", async () => {
    const prevStateDir = process.env.LOCALCLAW_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "localclaw-session-role-ordering-"));
    process.env.LOCALCLAW_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
