...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@localclaw"] },
      },
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "localclaw" explicitly).
    await capturedOnMessage?.({
      body: "localclaw ping",
      from: "123@g.us",
      conversationId: "123@g.us",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/localclaw-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/localclaw-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
