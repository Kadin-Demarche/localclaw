...
`;
    ...
    expect(parsed.localclaw.emoji).toBe("💾");
    expect(parsed.localclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.localclaw.emoji).toBe("📝");
    expect(parsed.localclaw.events).toEqual(["command"]);
    expect(parsed.localclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.localclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"localclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"localclaw": {"events": ["test"]}}');
  });
...
describe("resolveLocalClawMetadata", () => {
  it("extracts localclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveLocalClawMetadata(frontmatter);
  ...
  it("returns undefined when localclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        localclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with LocalClaw" },
            { id: "npm", kind: "npm", package: "@localclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@localclaw/hook");
  });
...
`;
    ...
    const localclaw = resolveLocalClawMetadata(frontmatter);
    expect(localclaw).toBeDefined();
    expect(localclaw?.emoji).toBe("💾");
    expect(localclaw?.events).toEqual(["command:new"]);
    expect(localclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(localclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const localclaw = resolveLocalClawMetadata(frontmatter);
    expect(localclaw?.emoji).toBe("disk");
    expect(localclaw?.events).toEqual(["command:new"]);
  });
});
