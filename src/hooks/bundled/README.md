...
This directory contains hooks that ship with LocalClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.localclaw.ai/hooks/soul-evil
...
Hooks can be configured in `~/.localclaw/localclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'localclaw.*gateway' && pnpm localclaw gateway`
3. Enable the hook: `localclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.localclaw.ai/hooks
