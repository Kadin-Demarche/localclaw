...
homepage: https://docs.localclaw.ai/hooks#command-logger
metadata:
  {
    "localclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.localclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.localclaw/logs/commands.log ~/.localclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/localclaw`:
   ```
   /home/username/.localclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
