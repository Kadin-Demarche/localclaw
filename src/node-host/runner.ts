...
const execHostEnforced = process.env.LOCALCLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.LOCALCLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureLocalClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
