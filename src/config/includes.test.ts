...
const ETC_LOCALCLAW_DIR = path.join(ROOT_DIR, "etc", "localclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "localclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcLocalClawPath(...parts: string[]) {
  return path.join(ETC_LOCALCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcLocalClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "localclaw.json"))).toEqual({
      shared: true,
    });
  });
});
