...
import type { LocalClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareLocalClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): LocalClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as LocalClawConfig;
}
...
function stampConfigVersion(cfg: LocalClawConfig): LocalClawConfig {
...
function warnIfConfigFromFuture(cfg: LocalClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareLocalClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer LocalClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: LocalClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): LocalClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as LocalClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as LocalClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: LocalClawConfig) {
    clearConfigCache();
...
// module scope. `LOCALCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.LOCALCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.LOCALCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): LocalClawConfig {
...
export async function writeConfigFile(cfg: LocalClawConfig): Promise<void> {
  clearConfigCache();
