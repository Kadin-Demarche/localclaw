...
import type { LocalClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: LocalClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: LocalClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LocalClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as LocalClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as LocalClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as LocalClawConfig)),
    ),
  };
}
