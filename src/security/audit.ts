  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your LocalClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your LocalClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectBrowserControlFindings(cfg: LocalClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("localclaw security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: LocalClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: LocalClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.LOCALCLAW_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.LOCALCLAW_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("localclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("localclaw security audit --deep")}".`,
    });
  }
