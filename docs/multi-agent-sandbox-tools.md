...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `localclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `localclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=localclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${LOCALCLAW_STATE_DIR:-$HOME/.localclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
