...
- `commands.config` (default `false`) enables `/config` (reads/writes `localclaw.json`).
...
- For full provider usage breakdown, use `localclaw status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from LocalClaw session logs.
...
- Overrides apply immediately to new config reads, but do **not** write to `localclaw.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`localclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[localclaw]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/localclaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
