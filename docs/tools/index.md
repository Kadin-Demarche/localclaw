---
summary: "Agent tool surface for LocalClaw (browser, canvas, nodes, message, cron) replacing legacy `localclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `localclaw-*` skills
title: "Tools"
---

# Tools (LocalClaw)

LocalClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `localclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `localclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, LocalClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`localclaw node run`).
...
- Requires a Brave API key (recommended: `localclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated LocalClaw-managed browser.
...
- Quick smoke: `localclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `localclaw gateway` restart in-place)
