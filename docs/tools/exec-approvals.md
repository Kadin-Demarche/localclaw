...
- **gateway host** → `localclaw` process on the gateway machine
...
`~/.localclaw/exec-approvals.json` directly.

CLI: `localclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
