---
summary: "Chrome extension: let LocalClaw drive your existing Chrome tab"
read_when:
...
The LocalClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate localclaw-managed Chrome profile.
...
LocalClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the LocalClaw release (npm package) as static files. There is no separate “build” step.

After upgrading LocalClaw:

- Re-run `localclaw browser extension install` to refresh the installed files under your LocalClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

LocalClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want LocalClaw to control.
...
- `ON`: attached; LocalClaw can drive that tab.
...
Debugging: `localclaw sandbox explain`
...
`localclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `localclaw browser extension install` first to copy the extension to a stable location under your LocalClaw state directory.
...
- **This is not isolated** like the dedicated localclaw-managed profile.
