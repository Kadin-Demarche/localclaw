...
You can also set `LOCALCLAW_VERBOSE=1` instead of the flag.
...
### `docs.localclaw.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.localclaw.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.localclaw.ai` to the allowlist, then retry.
...
- Check the allowlist: `localclaw config get agents.defaults.models`
...
If you can, include the relevant log tail from `localclaw logs --follow`.
