...
LocalClaw features that can generate provider usage or paid API calls.
...
- `localclaw status --usage` and `localclaw channels list` show provider **usage windows**
...
LocalClaw can pick up credentials from:
...
- `localclaw status --usage`
- `localclaw models status --json`
...
`localclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
