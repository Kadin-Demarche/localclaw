...
This document explains how LocalClaw manages sessions end-to-end:
...
LocalClaw is designed around a single **Gateway process** that owns session state.
...
LocalClaw persists sessions in two layers:
...
- Transcripts: `~/.localclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

LocalClaw resolves these via `src/config/sessions.ts`.
...
LocalClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (LocalClaw consumes the events, but Pi decides when to compact).
...
LocalClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, LocalClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, LocalClaw leaves it alone.
...
- `localclaw status` (CLI)
- `localclaw sessions` / `sessions --json`
...
LocalClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- LocalClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, LocalClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
LocalClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but LocalClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `localclaw status`.
