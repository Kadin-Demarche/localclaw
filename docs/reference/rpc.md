...
LocalClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- LocalClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- LocalClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
