...
- Publish as `localclaw/plugin-sdk` (or export from core under `localclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `LocalClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: LocalClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: LocalClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: LocalClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: LocalClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: LocalClawConfig): string;
  };
};
```
...
- Introduce `localclaw/plugin-sdk`.
- Add `api.runtime` to `LocalClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `localclawRuntime: ">=2026.2.0"`).
