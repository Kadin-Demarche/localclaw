...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.localclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.localclaw/node.json` (node id + pairing token).
...
Path: `~/.localclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.localclaw/exec-approvals.sock",
...
- Unix socket at `~/.localclaw/exec-approvals.sock` (0600).
