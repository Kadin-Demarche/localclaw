---
summary: "OpenProse: .prose workflows, slash commands, and state in LocalClaw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In LocalClaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
localclaw plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `localclaw plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses LocalClaw tools under the hood.
...
## LocalClaw runtime mapping

OpenProse programs map to LocalClaw primitives:

| OpenProse concept         | LocalClaw tool   |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use LocalClaw tool allowlists and approval gates to control side effects.
