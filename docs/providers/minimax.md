---
summary: "Use MiniMax M2.1 in LocalClaw"
read_when:
  - You want MiniMax models in LocalClaw
...
```bash
localclaw plugins enable minimax-portal-auth  # skip if already loaded.
localclaw gateway restart  # restart if gateway is already running
localclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/localclaw/localclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `localclaw.json`:
...
- Use `localclaw models list` and `localclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `localclaw configure` and selecting **MiniMax M2.1**, or
