---
summary: "Use Venice AI privacy-focused models in LocalClaw"
read_when:
  - You want privacy-focused inference in LocalClaw
...
```bash
localclaw onboard --auth-choice venice-api-key
```
...
```bash
localclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, LocalClaw shows all available Venice models. Pick based on your needs:
...
```bash
localclaw models set venice/claude-opus-45
localclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
localclaw models list | grep venice
```

## Configure via `localclaw configure`

1. Run `localclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
LocalClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
localclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
localclaw chat --model venice/claude-opus-45

# Use uncensored model
localclaw chat --model venice/venice-uncensored

# Use vision model with image
localclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
localclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `localclaw models list` to see currently available models. Some models may be temporarily offline.
