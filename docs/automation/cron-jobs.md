...
- Jobs persist under `~/.localclaw/cron/` so restarts don’t lose schedules.
...
localclaw cron list
localclaw cron run <job-id>
localclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.localclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `localclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, LocalClaw defaults to `announce`.
...
- Job store: `~/.localclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.localclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.localclaw/cron/jobs.json",
...
localclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
localclaw cron edit <jobId> --agent ops
localclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
localclaw cron run <jobId>
localclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
localclaw cron edit <jobId> \
...
```bash
localclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`localclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `LOCALCLAW_SKIP_CRON`.
