...
Deploy LocalClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/localclaw` — Control UI
...
- `LOCALCLAW_STATE_DIR=/data/.localclaw` (recommended)
- `LOCALCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `LOCALCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your LocalClaw state + workspace so you can migrate to another host without losing config or memory.
