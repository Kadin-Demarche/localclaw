...
Deploy LocalClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/localclaw/localclaw)
...
repository configures everything needed to run LocalClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: LOCALCLAW_STATE_DIR
        value: /data/.localclaw
      - key: LOCALCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: LOCALCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/localclaw`.
...
If you use the original LocalClaw repository, Render will not auto-deploy your LocalClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For LocalClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any LocalClaw host.
