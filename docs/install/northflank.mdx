...
Deploy LocalClaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy LocalClaw](https://northflank.com/stacks/deploy-localclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy LocalClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the LocalClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the LocalClaw service.
8. Open the public LocalClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/localclaw`.

## What you get

- Hosted LocalClaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/localclaw`
