...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “LocalClaw runs”).
...
- `localclaw health` and `localclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `localclaw gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`LOCALCLAW_GATEWAY_PORT`).
