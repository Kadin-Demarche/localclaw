...
- During heartbeat runs, LocalClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `localclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, LocalClaw skips the heartbeat run entirely (no model call).
...
```bash
localclaw system event --text "Check for urgent follow-ups" --mode now
```
