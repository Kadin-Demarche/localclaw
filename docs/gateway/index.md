...
- Replaces the legacy `gateway` command. CLI entry point: `localclaw gateway`.
...
localclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.localclaw/localclaw.json` (or `LOCALCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__localclaw__/canvas/` from `~/.localclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `LOCALCLAW_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `LOCALCLAW_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `LOCALCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.localclaw.*` may still exist)
- Linux: `localclaw-gateway-<profile>.service`
- Windows: `LocalClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `LOCALCLAW_SERVICE_MARKER=localclaw`
- `LOCALCLAW_SERVICE_KIND=gateway`
- `LOCALCLAW_SERVICE_VERSION=<version>`
...
```bash
localclaw --dev setup
localclaw --dev gateway --allow-unconfigured
# then target the dev instance:
localclaw --dev status
localclaw --dev health
```

Defaults (can be overridden via env/flags/config):

- `LOCALCLAW_STATE_DIR=~/.localclaw-dev`
- `LOCALCLAW_CONFIG_PATH=~/.localclaw-dev/localclaw.json`
- `LOCALCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.localclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `LOCALCLAW_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `LOCALCLAW_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `LOCALCLAW_CONFIG_PATH`
- unique `LOCALCLAW_STATE_DIR`
...
```bash
localclaw --profile main gateway install
localclaw --profile rescue gateway install
```

Example:

```bash
LOCALCLAW_CONFIG_PATH=~/.localclaw/a.json LOCALCLAW_STATE_DIR=~/.localclaw-a localclaw gateway --port 19001
LOCALCLAW_CONFIG_PATH=~/.localclaw/b.json LOCALCLAW_STATE_DIR=~/.localclaw-b localclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `localclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `localclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `localclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.localclaw.*` is cleaned up).
  - `localclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are LocalClaw profile services.
  ...
  - Cleanup: `localclaw gateway uninstall` (current service) and `localclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `localclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- LocalClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.localclaw.*` labels still unload cleanly).
- To stop it cleanly, use `localclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `localclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `localclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

LocalClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`localclaw gateway install` writes the user unit. `localclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/localclaw-gateway[-<profile>].service`:

```
[Unit]
Description=LocalClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/localclaw gateway --port 18789
Restart=always
RestartSec=5
Environment=LOCALCLAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now localclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/localclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now localclaw-gateway[-<profile>].service
```
...
- `localclaw gateway health|status` — request health/status over the Gateway WS.
- `localclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `localclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `localclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `localclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `localclaw gateway` and the legacy TCP control port.
