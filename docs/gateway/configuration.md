---
summary: "All configuration options for ~/.localclaw/localclaw.json with examples"
read_when:
...
LocalClaw reads an optional **JSON5** config from `~/.localclaw/localclaw.json` (comments + trailing commas allowed).

If the file is missing, LocalClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.localclaw/workspace`). You usually only need a config to:
...
LocalClaw only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `localclaw doctor`, `localclaw logs`, `localclaw health`, `localclaw status`, `localclaw service`, `localclaw help`).
- Run `localclaw doctor` to see the exact issues.
- Run `localclaw doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `localclaw config set`. Keep a backup of `~/.localclaw/localclaw.json`.
...
```bash
localclaw gateway call config.get --params '{}' # capture payload.hash
localclaw gateway call config.apply --params '{
...
```bash
localclaw gateway call config.get --params '{}' # capture payload.hash
localclaw gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.localclaw/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@localclaw", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.localclaw/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.localclaw/workspace" }],
}
```
...
{ "$include": "/etc/localclaw/base.json5" } // absolute
...
```json5
// ~/.localclaw/clients/mueller/agents.json5
[
...
```json5
// ~/.localclaw/clients/mueller/broadcast.json5
{
...
LocalClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, LocalClaw runs your login shell and imports only the missing expected keys (never overrides).
...
- `LOCALCLAW_LOAD_SHELL_ENV=1`
- `LOCALCLAW_SHELL_ENV_TIMEOUT_MS=15000`
...
LocalClaw stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.localclaw/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.localclaw/credentials/oauth.json` (or `$LOCALCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.localclaw/agent/*` (migrated by `localclaw doctor` into `~/.localclaw/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `LOCALCLAW_OAUTH_DIR`
- Agent dir (default agent root override): `LOCALCLAW_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, LocalClaw imports `oauth.json` entries into `auth-profiles.json`.
...
If set, LocalClaw derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/localclaw/localclaw-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/localclaw/localclaw.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/localclaw/localclaw.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.localclaw/credentials/whatsapp/biz
          // authDir: "~/.localclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `localclaw doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@localclaw", "localclaw"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@localclaw"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.localclaw/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.localclaw/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.localclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.localclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.localclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.localclaw/workspace-home" },
      { id: "work", workspace: "~/.localclaw/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `localclaw.json`).
...
LocalClaw starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["localclaw-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-localclaw",
...
LocalClaw starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `localclaw plugins install @localclaw/mattermost` (or `./extensions/mattermost` from a git checkout).
...
LocalClaw starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
LocalClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, LocalClaw
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/localclaw" } },
}
```
...
When a file exceeds this limit, LocalClaw logs a warning and injects a truncated
...
message envelopes). If unset, LocalClaw uses the host timezone at runtime.
...
`[localclaw]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[localclaw]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[localclaw]"`, LocalClaw will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, LocalClaw generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.localclaw/settings/tts.json",
      elevenlabs: {
...
LocalClaw also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, LocalClaw currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `localclaw configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:localclaw`: all built-in LocalClaw tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.localclaw/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.localclaw/sandboxes",
        docker: {
          image: "localclaw-sandbox:bookworm-slim",
          containerPrefix: "localclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "localclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "localclaw-sandbox-browser:bookworm-slim",
          containerPrefix: "localclaw-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
LocalClaw uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.localclaw/agents/<agentId>/agent/models.json` or by defining the same schema inside your
LocalClaw config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, LocalClaw writes/merges a `models.json` into
`~/.localclaw/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. LocalClaw uses
...
- Shortcut: `localclaw onboard --auth-choice opencode-zen`.
...
Shortcut: `localclaw onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `localclaw onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `localclaw onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `localclaw onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `localclaw onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `localclaw onboard --auth-choice minimax-api`.
...
- Override the agent config root with `LOCALCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.localclaw/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.localclaw/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, LocalClaw stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.localclaw/skills` (workspace skills
...
from `~/.localclaw/extensions`, `<workspace>/.localclaw/extensions`, plus any
...
### `browser` (localclaw-managed browser)

LocalClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for localclaw and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (LocalClaw.app menubar, or `localclaw gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      localclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/localclaw" }
...
- Examples: `"/ui"`, `"/localclaw"`, `"/apps/localclaw"`.
...
- When a connection comes from one of these IPs, LocalClaw uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `localclaw gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `LOCALCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `LOCALCLAW_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. LocalClaw
...
- LocalClaw.app watches `~/.localclaw/localclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.localclaw/localclaw.json` (or `LOCALCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.localclaw/localclaw.json` (or `LOCALCLAW_CONFIG_PATH`)
...
- `LOCALCLAW_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `localclaw --dev …` → uses `~/.localclaw-dev` + shifts ports from base `19001`
- `localclaw --profile <name> …` → uses `~/.localclaw-<name>` (port via config/env/flags)
...
```bash
LOCALCLAW_CONFIG_PATH=~/.localclaw/a.json \
LOCALCLAW_STATE_DIR=~/.localclaw-a \
localclaw gateway --port 19001
```
...
Gmail helper config (used by `localclaw webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "localclaw@gmail.com",
...
- Set `LOCALCLAW_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, LocalClaw defaults `serve.path` to `/` so
...
Default root: `~/.localclaw/workspace/canvas`  
Default port: `18793` (chosen to avoid the localclaw browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__localclaw__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__localclaw__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.localclaw/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `LOCALCLAW_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.localclaw/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.localclaw/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.localclaw/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_localclaw-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `localclaw` (advertises `localclaw.local`). Override with `LOCALCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_localclaw-gw._tcp` under `~/.localclaw/dns/` using the configured discovery domain (example: `localclaw.internal.`).
