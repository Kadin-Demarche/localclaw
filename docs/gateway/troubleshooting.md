---
summary: "Quick troubleshooting guide for common LocalClaw failures"
read_when:
...
When LocalClaw misbehaves, here's how to fix it.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `localclaw status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `localclaw status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `localclaw status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `localclaw gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `localclaw channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `localclaw gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `localclaw logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `localclaw status --all` (it redacts tokens). If you paste `localclaw status`, consider setting `LOCALCLAW_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ...
  ```bash
  localclaw models auth setup-token --provider anthropic
  ```
...
```bash
localclaw models auth paste-token --provider anthropic
localclaw models status
```
...
- Preferred: `localclaw logs --follow`
- File logs (always): `/tmp/localclaw/localclaw-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$LOCALCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u localclaw-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ...
  ```bash
  localclaw config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ...
  ```bash
  localclaw config set gateway.mode remote
  localclaw config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `localclaw setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.localclaw/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `localclaw doctor`
...
- then run `localclaw sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `localclaw gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `localclaw gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `localclaw gateway status` and `localclaw doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `LOCALCLAW_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `localclaw gateway status` shows a config mismatch**
...
- Fix: rerun `localclaw gateway install --force` from the same `--profile` / `LOCALCLAW_STATE_DIR` you want the service to use.

**If `localclaw gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `localclaw doctor` to update it (or `localclaw gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `LOCALCLAW_GATEWAY_TOKEN`) and restart the service.

**If `localclaw gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/localclaw` on disk.
...
Symptoms: `pwd` or file tools show `~/.localclaw/sandboxes/...` even though you
...
LocalClaw intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `localclaw models list` or
  `localclaw models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${LOCALCLAW_CONFIG_PATH:-$HOME/.localclaw/localclaw.json}"
```
...
tail -f "$(ls -t /tmp/localclaw/localclaw-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
localclaw pairing list <channel>
```
...
```bash
localclaw logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.localclaw/agents/<agentId>/sessions/
```
...
localclaw logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
localclaw channels logout
trash "${LOCALCLAW_STATE_DIR:-$HOME/.localclaw}/credentials" # if logout can't cleanly remove everything
localclaw channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/localclaw/localclaw-*.log | head -1)" | tail -20
```

### High Memory Usage

LocalClaw keeps conversation history in memory.
...
LocalClaw now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
```bash
localclaw doctor
localclaw doctor --fix
```

Notes:

- `localclaw doctor` reports every invalid entry.
- `localclaw doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `localclaw logs`, `localclaw health`, `localclaw status`, `localclaw gateway status`, and `localclaw gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/localclaw/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `localclaw models status` (CLI).
...
5. Run `localclaw channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. LocalClaw scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/localclaw/localclaw/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.localclaw.* still works)
```
...
Ensure the global `localclaw` CLI is installed and matches the app version:

```bash
localclaw --version
npm install -g localclaw@<version>
```
...
#   ${LOCALCLAW_CONFIG_PATH:-$HOME/.localclaw/localclaw.json} -> { logging: { level: "trace" } }
#
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                         |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/localclaw/localclaw-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                    |
| Gateway service logs (supervisor) | macOS: `$LOCALCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.localclaw/logs/...`; profiles use `~/.localclaw-<profile>/logs/...`)<br />Linux: `journalctl --user -u localclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "LocalClaw Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$LOCALCLAW_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                                |
| Media cache                       | `$LOCALCLAW_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                    |
| Credentials                       | `$LOCALCLAW_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                              |
...
localclaw gateway status --deep

# Is the gateway reachable?
localclaw health --json
# If it fails, rerun with connection details:
localclaw health --verbose
...
tail -20 /tmp/localclaw/localclaw-*.log
```
...
trash "${LOCALCLAW_STATE_DIR:-$HOME/.localclaw}"
localclaw channels login         # re-pair WhatsApp
localclaw gateway restart           # or: localclaw gateway
```
...
1. Check logs first: `/tmp/localclaw/` (default: `localclaw-YYYY-MM-DD.log`, or your configured `logging.file`)
