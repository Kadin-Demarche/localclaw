...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/localclaw`)
...
If the page fails to load, start the Gateway first: `localclaw gateway`.
...
you revoke it with `localclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.localclaw/localclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. LocalClaw
...
LocalClaw **blocks** Control UI connections without device identity.
