...
read_when:
  - Designing workspace memory (~/.localclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep LocalClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.localclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.localclaw/workspace` as canonical human-readable memory.
...
```
~/.localclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in LocalClaw**, but keep a separable core library.

### Why integrate into LocalClaw?

- LocalClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `localclaw memory recall "…" --k 25 --since 30d`
  - `localclaw memory reflect --since 7d`
...
Pragmatic take for `~/.localclaw/workspace`:
