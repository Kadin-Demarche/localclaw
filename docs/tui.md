...
```bash
localclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `localclaw tui --deliver`
...
- Check the Gateway logs: `localclaw logs --follow`.
- Confirm the agent can run: `localclaw status` and `localclaw models status`.
...
- No agents in picker: check `localclaw agents list` and your routing config.
