...
read_when:
  - You need detailed behavior for localclaw onboard
...
This page is the full reference for `localclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.localclaw/localclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `localclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `localclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `localclaw health`.
    - `localclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.localclaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.localclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.localclaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`localclaw onboard`](/cli/onboard)
