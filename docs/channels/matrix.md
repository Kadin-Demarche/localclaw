...
Matrix is an open, decentralized messaging protocol. LocalClaw connects as a Matrix **user**
...
```bash
localclaw plugins install @localclaw/matrix
```

Local checkout (when running from a git repo):

```bash
localclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `localclaw plugins install @localclaw/matrix`
   - From a local checkout: `localclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: LocalClaw calls the same
     login endpoint, stores the access token in `~/.localclaw/credentials/matrix/credentials.json`,
...
- On first connection, LocalClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  LocalClaw logs a warning.
...
`~/.localclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `localclaw pairing list matrix`
  - `localclaw pairing approve matrix <CODE>`
...
- On startup, LocalClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
