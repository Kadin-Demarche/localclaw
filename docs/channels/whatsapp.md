...
2. Configure WhatsApp in `~/.localclaw/localclaw.json`.
3. Run `localclaw channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run LocalClaw on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for LocalClaw. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the LocalClaw number there.
...
Quick fallback: run LocalClaw on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[localclaw]`)
...
- Early LocalClaw builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `localclaw channels login` (QR via Linked Devices).
- Multi-account login: `localclaw channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.localclaw/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.localclaw/credentials/`.
- Logout: `localclaw channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `localclaw pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run LocalClaw on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will LocalClaw message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. LocalClaw only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `localclaw pairing approve whatsapp <code>` (list with `localclaw pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different LocalClaw instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `localclaw message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. LocalClaw rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/localclaw/localclaw-YYYY-MM-DD.log` (configurable).
...
- Fix: run `localclaw channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `localclaw doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `localclaw logs --follow`.
