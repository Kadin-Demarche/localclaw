...
read_when:
  - Setting up Twitch chat integration for LocalClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. LocalClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
localclaw plugins install @localclaw/twitch
```

Local checkout (when running from a git repo):

```bash
localclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `LOCALCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "localclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use LOCALCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
LOCALCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
