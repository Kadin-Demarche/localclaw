...
Tlon is a decentralized messenger built on Urbit. LocalClaw connects to your Urbit ship and can
...
```bash
localclaw plugins install @localclaw/tlon
```

Local checkout (when running from a git repo):

```bash
localclaw plugins install ./extensions/tlon
```
...
Use these with `localclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, LocalClaw replies in-thread.
