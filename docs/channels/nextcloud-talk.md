...
```bash
localclaw plugins install @localclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
localclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "LocalClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `localclaw pairing list nextcloud-talk`
  - `localclaw pairing approve nextcloud-talk <CODE>`
