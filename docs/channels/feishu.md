...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects LocalClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
localclaw plugins install @localclaw/feishu
```

Local checkout (when running from a git repo):

```bash
localclaw plugins install ./extensions/feishu
```
...
If you just installed LocalClaw, run the wizard:
...
2. Configuring app credentials in LocalClaw
...
1. You already ran `localclaw channels add` for Feishu
2. The gateway is running (`localclaw gateway status`)
...
```bash
localclaw pairing approve feishu <CODE>
```
...
2. Run `localclaw logs --follow` and look for `chat_id`
...
2. Run `localclaw logs --follow` and look for `open_id`
...
| Command                     | Description                   |
| --------------------------- | ----------------------------- |
| `localclaw gateway status`  | Show gateway status           |
| `localclaw gateway install` | Install/start gateway service |
| `localclaw gateway stop`    | Stop gateway service          |
| `localclaw gateway restart` | Restart gateway service       |
| `localclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `localclaw logs --follow`
...
5. Ensure the gateway is running: `localclaw gateway status`
6. Check logs: `localclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.localclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.localclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
