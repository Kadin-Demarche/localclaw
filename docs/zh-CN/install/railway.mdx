...
通过一键模板在 Railway 上部署 LocalClaw，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/localclaw` — 控制面板 UI
...
- `LOCALCLAW_STATE_DIR=/data/.localclaw`（推荐）
- `LOCALCLAW_WORKSPACE_DIR=/data/workspace`（推荐）
- `LOCALCLAW_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 LocalClaw 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
