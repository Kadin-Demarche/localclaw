...
如果您刚安装完 LocalClaw，可以直接运行向导，根据提示添加飞书：
...
- `localclaw gateway status` - 查看网关运行状态
- `localclaw logs --follow` - 查看实时日志
...
- `localclaw gateway status` - 查看网关运行状态
- `localclaw gateway restart` - 重启网关以应用新配置
- `localclaw logs --follow` - 查看实时日志
...
1. 运行 `localclaw channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `localclaw gateway status` 检查状态）
...
```bash
localclaw pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  localclaw pairing list feishu      # 查看待审批列表
  localclaw pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `localclaw logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `localclaw logs --follow` 查看日志中的 `open_id`
...
| --------------------------- | ----------------- |
| `localclaw gateway status`  | 查看网关运行状态  |
| `localclaw gateway install` | 安装/启动网关服务 |
| `localclaw gateway stop`    | 停止网关服务      |
| `localclaw gateway restart` | 重启网关服务      |
| `localclaw logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`localclaw gateway status`
6. 查看实时日志：`localclaw logs --follow`
