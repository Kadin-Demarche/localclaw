...
- 通过 CLI 安装：`localclaw plugins install @localclaw/zalouser`
- 或从源码检出安装：`localclaw plugins install ./extensions/zalouser`
- 详情：[插件](/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，LocalClaw 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`localclaw channels logout --channel zalouser && localclaw channels login --channel zalouser`
