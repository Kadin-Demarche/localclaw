...
你可以在 `~/.localclaw/localclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/localclaw.log"
  }
}
```
...
所有日志配置都在 `~/.localclaw/localclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/localclaw/localclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- LocalClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
LOCALCLAW_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `localclaw plugins enable diagnostics-otel` 启用插件。
...
- `localclaw.tokens`（计数器，属性：`localclaw.token`、`localclaw.channel`、`localclaw.provider`、`localclaw.model`）
- `localclaw.cost.usd`（计数器，属性：`localclaw.channel`、`localclaw.provider`、`localclaw.model`）
- `localclaw.run.duration_ms`（直方图，属性：`localclaw.channel`、`localclaw.provider`、`localclaw.model`）
- `localclaw.context.tokens`（直方图，属性：`localclaw.context`、`localclaw.channel`、`localclaw.provider`、`localclaw.model`）

消息流：

- `localclaw.webhook.received`（计数器，属性：`localclaw.channel`、`localclaw.webhook`）
- `localclaw.webhook.error`（计数器，属性：`localclaw.channel`、`localclaw.webhook`）
- `localclaw.webhook.duration_ms`（直方图，属性：`localclaw.channel`、`localclaw.webhook`）
- `localclaw.message.queued`（计数器，属性：`localclaw.channel`、`localclaw.source`）
- `localclaw.message.processed`（计数器，属性：`localclaw.channel`、`localclaw.outcome`）
- `localclaw.message.duration_ms`（直方图，属性：`localclaw.channel`、`localclaw.outcome`）

队列 + 会话：

- `localclaw.queue.lane.enqueue`（计数器，属性：`localclaw.lane`）
- `localclaw.queue.lane.dequeue`（计数器，属性：`localclaw.lane`）
- `localclaw.queue.depth`（直方图，属性：`localclaw.lane` 或 `localclaw.channel=heartbeat`）
- `localclaw.queue.wait_ms`（直方图，属性：`localclaw.lane`）
- `localclaw.session.state`（计数器，属性：`localclaw.state`、`localclaw.reason`）
- `localclaw.session.stuck`（计数器，属性：`localclaw.state`）
- `localclaw.session.stuck_age_ms`（直方图，属性：`localclaw.state`）
- `localclaw.run.attempt`（计数器，属性：`localclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `localclaw.model.usage`
  - `localclaw.channel`、`localclaw.provider`、`localclaw.model`
  - `localclaw.sessionKey`、`localclaw.sessionId`
  - `localclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `localclaw.webhook.processed`
  - `localclaw.channel`、`localclaw.webhook`、`localclaw.chatId`
- `localclaw.webhook.error`
  - `localclaw.channel`、`localclaw.webhook`、`localclaw.chatId`、`localclaw.error`
- `localclaw.message.processed`
  - `localclaw.channel`、`localclaw.outcome`、`localclaw.chatId`、`localclaw.messageId`、`localclaw.sessionKey`、`localclaw.sessionId`、`localclaw.reason`
- `localclaw.session.stuck`
  - `localclaw.state`、`localclaw.ageMs`、`localclaw.queueDepth`、`localclaw.sessionKey`、`localclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `localclaw doctor`。
