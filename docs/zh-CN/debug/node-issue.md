...
read_when:
  ...
  - 排查 LocalClaw 中 tsx/esbuild 加载器崩溃问题
...
通过 Node 使用 `tsx` 运行 LocalClaw 时，启动阶段报错：
...
- 在此之前（Bun 路径），`localclaw status` 和 `gateway:watch` 均正常工作。
...
- 使用 Node + tsc watch，然后运行编译产物：
  ...
  ```
- 已在本地确认：`pnpm exec tsc -p tsconfig.json` + `node localclaw.mjs status` 在 Node 25 上可正常运行。
