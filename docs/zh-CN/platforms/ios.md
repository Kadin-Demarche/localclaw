...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`localclaw.internal.`），**或**
  - 手动主机/端口（备选）。
...
Gateway 网关在 `local.` 上广播 `_localclaw-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`localclaw.internal.`）和 Tailscale 分割 DNS。
...
- Gateway 网关 canvas 主机服务于 `/__localclaw__/canvas/` 和 `/__localclaw__/a2ui/`。
...
- 配对提示从未出现：运行 `localclaw nodes pending` 并手动批准。
