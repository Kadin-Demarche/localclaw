...
LocalClaw 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`localclaw gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`localclaw onboard --install-daemon`
- 直接安装：`localclaw gateway install`
- 配置流程：`localclaw configure` → 选择 **Gateway service**
- 修复/迁移：`localclaw doctor`（提供安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.localclaw.*`）
- Linux/WSL2：systemd 用户服务（`localclaw-gateway[-<profile>].service`）
