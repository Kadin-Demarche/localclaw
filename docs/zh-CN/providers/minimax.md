---
read_when:
  - 你想在 LocalClaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 LocalClaw 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
localclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
localclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/localclaw/localclaw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `localclaw models list` 和 `localclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `localclaw configure` 并选择 **MiniMax M2.1**，或
