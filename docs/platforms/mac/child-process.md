...
service via the external `localclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`LOCALCLAW_PROFILE`; legacy `com.localclaw.*` is supported).
