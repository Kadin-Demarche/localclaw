---
summary: "LocalClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up LocalClaw on a Raspberry Pi
  - Running LocalClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# LocalClaw on Raspberry Pi

## Goal

Run a persistent, always-on LocalClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most LocalClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled localclaw

# Enable if not
sudo systemctl enable localclaw

# Start on boot
sudo systemctl start localclaw
```
...
```bash
# Check logs
journalctl -u localclaw --no-pager -n 100

# Common fix: rebuild
cd ~/localclaw  # if using hackable install
npm run build
sudo systemctl restart localclaw
```
