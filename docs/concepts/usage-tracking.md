...
- `/usage cost` in chats: local cost summary aggregated from LocalClaw session logs.
- CLI: `localclaw status --usage` prints a full per-provider breakdown.
- CLI: `localclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
