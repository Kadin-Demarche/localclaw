...
read_when:
  - You want to understand what “context” means in LocalClaw
...
“Context” is **everything LocalClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (LocalClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How LocalClaw builds the system prompt

The system prompt is **LocalClaw-owned** and rebuilt each run. It includes:
...
By default, LocalClaw injects a fixed set of workspace files (if present):
