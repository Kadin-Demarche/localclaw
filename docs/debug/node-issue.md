...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in LocalClaw
...
Running LocalClaw via Node with `tsx` fails at startup with:

```
[localclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `localclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch localclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node localclaw.mjs status` works on Node 25.
