...
import type { LocalClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("localclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("localclaw-bin-sherpa-");
      const modelDir = await makeTempDir("localclaw-sherpa-model-");
      ...
      const cfg: LocalClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("localclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("localclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("localclaw-bin-gemini-");
      ...
      const cfg: LocalClawConfig = { tools: { media: { image: {} } } };
