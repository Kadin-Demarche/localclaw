...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("LOCALCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.LOCALCLAW_STATE_DIR = path.join(base, ".localclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "localclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".localclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
