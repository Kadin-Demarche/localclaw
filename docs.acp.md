# LocalClaw ACP Bridge

This document describes how the LocalClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`localclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
LocalClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a LocalClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `localclaw acp` over stdio.

Example config:

```bash
localclaw config set gateway.remote.url wss://gateway-host:18789
localclaw config set gateway.remote.token <token>
```

Example run:

```bash
localclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
localclaw acp --session agent:main:main
localclaw acp --session agent:design:main
localclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “LocalClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `localclaw acp` and speaks ACP messages over stdio.
...
```bash
localclaw acp --session agent:main:main
localclaw acp --session-label "support inbox"
localclaw acp --reset-session
```
...
`localclaw acp` resolves the Gateway URL and auth from CLI flags or config:
