...
metadata:
  { "localclaw": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (LocalClaw)
...
```bash
SOCKET_DIR="${LOCALCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/localclaw-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/localclaw.sock"
SESSION=localclaw-python
...
- Use `LOCALCLAW_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$LOCALCLAW_TMUX_SOCKET_DIR/localclaw.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `LOCALCLAW_TMUX_SOCKET_DIR`).
