---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for LocalClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, LocalClaw cron scheduling for periodic checks, or version status checks on a machine running LocalClaw (laptop, workstation, Pi, VPS).
---

# LocalClaw Host Hardening

## Overview

Assess and harden the host running LocalClaw, then align it to a user-defined risk tolerance without breaking access. Use LocalClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim LocalClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. LocalClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if LocalClaw can access sensitive data.
...
### 2) Run LocalClaw security audits (read-only)

As part of the default read-only checks, run `localclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `localclaw security audit` (faster, non-probing)
2. `localclaw security audit --json` (structured output)

Offer to apply LocalClaw safe defaults (numbered):

1. `localclaw security audit --fix`

Be explicit that `--fix` only tightens LocalClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check LocalClaw version/update status (read-only)

As part of the default read-only checks, run `localclaw update status`.
...
- Credential hygiene notes (location of LocalClaw creds, prefer disk encryption)
...
- LocalClaw security audit (re-run)
...
After LocalClaw install or first hardening pass, run at least one baseline audit and version check:

- `localclaw security audit`
- `localclaw security audit --deep`
- `localclaw update status`

Ongoing monitoring is recommended. Use the LocalClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `localclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `localclaw update status`
...
Before creating, `localclaw cron list` and match on exact `name`. If found, `localclaw cron edit <id> ...`.
If not found, `localclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `localclaw update status` (preferred for source checkouts and channels)
2. `npm view localclaw version` (published npm version)

## LocalClaw command accuracy

Use only supported commands and flags:

- `localclaw security audit [--deep] [--fix] [--json]`
- `localclaw status` / `localclaw status --deep`
- `localclaw health --json`
- `localclaw update status`
- `localclaw cron add|list|runs|run`

Do not invent CLI flags or imply LocalClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by LocalClaw compaction:
