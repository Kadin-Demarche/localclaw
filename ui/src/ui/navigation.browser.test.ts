...
import { LocalClawApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = LocalClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("localclaw-app") as LocalClawApp;
...
beforeEach(() => {
  LocalClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__LOCALCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  LocalClawApp.prototype.connect = originalConnect;
  window.__LOCALCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/localclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/localclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/localclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__LOCALCLAW_CONTROL_UI_BASE_PATH__ = "/localclaw";
    const app = mountApp("/localclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/localclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/localclaw/sessions");
  });
