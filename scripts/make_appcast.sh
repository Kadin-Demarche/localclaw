...
ZIP=${1:?"Usage: $0 LocalClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/localclaw/localclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^LocalClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/localclaw/localclaw/releases/download/v${VERSION}/"}
