...
INSTALL_URL="${LOCALCLAW_INSTALL_URL:-https://localclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${LOCALCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${LOCALCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="localclaw"
PACKAGE_NAME="${LOCALCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (LOCALCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${LOCALCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${LOCALCLAW_INSTALL_LATEST_OUT:-}"
fi
