#!/bin/bash
# Setup LocalClaw Auth Management System
...
echo "=== LocalClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'localclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  LocalClaw message: Send warning via LocalClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/localclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/localclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/localclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now localclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status localclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
