...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "LocalClaw")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "LocalClaw")
...
DMG_TEMP="$(mktemp -d /tmp/localclaw-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/LocalClaw/Resources/LocalClaw.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/localclaw-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/localclaw-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/localclaw-dmg-limits.txt
