...
function runTests() {
  const isolatedLock =
    process.env.LOCALCLAW_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `localclaw-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      LOCALCLAW_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.LOCALCLAW_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
