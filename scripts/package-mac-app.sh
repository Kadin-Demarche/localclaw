...
# Build and bundle LocalClaw into a minimal .app we can open.
# Outputs to dist/LocalClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/LocalClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="LocalClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.localclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/localclaw/localclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/LocalClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :LocalClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :LocalClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/LocalClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/LocalClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/LocalClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/LocalClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/LocalClaw/Resources/LocalClaw.icns" "$APP_ROOT/Contents/Resources/LocalClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/LocalClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying LocalClawKit resources"
LOCALCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/LocalClawKit_LocalClawKit.bundle"
if [ -d "$LOCALCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/LocalClawKit_LocalClawKit.bundle"
  cp -R "$LOCALCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/LocalClawKit_LocalClawKit.bundle"
else
  echo "WARN: LocalClawKit resource bundle not found at $LOCALCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running LocalClaw"
killall -q LocalClaw 2>/dev/null || true
