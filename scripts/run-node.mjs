...
const shouldBuild = () => {
  if (env.LOCALCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.LOCALCLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[localclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["localclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
