 ...
 * `localclaw update`. Use it to verify the completion installation flow
...
\`localclaw update\`, \`localclaw doctor\`, and \`localclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# LocalClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`localclaw completion --install\` later to enable.`));
    return;
  }
