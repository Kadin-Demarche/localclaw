...
#
# Suggested cron: */30 * * * * /home/admin/localclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send LocalClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., localclaw-alerts)
...
STATE_FILE="$HOME/.localclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via LocalClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use localclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via LocalClaw to $NOTIFY_PHONE..."
            localclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! LocalClaw is down. Run: ssh l36 '~/localclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
