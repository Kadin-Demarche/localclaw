...
IMAGE_NAME="localclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="localclaw-net-e2e-$$"
GW_NAME="localclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "LOCALCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "LOCALCLAW_SKIP_CHANNELS=1" \
	  -e "LOCALCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "LOCALCLAW_SKIP_CRON=1" \
	  -e "LOCALCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
