...
IMAGE_NAME="localclaw-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/localclaw-bin:$PATH"
  mkdir -p /tmp/localclaw-bin

  cat > /tmp/localclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/localclaw-bin/systemctl

  cat > /tmp/localclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/localclaw-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/localclaw"
  npm_entry="/tmp/npm-prefix/lib/node_modules/localclaw/dist/index.js"
  git_entry="/app/dist/index.js"
  git_cli="/app/localclaw.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/localclaw-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/localclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
