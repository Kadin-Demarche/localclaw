...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/localclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "LocalClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/localclaw/scripts/mobile-reauth.sh'" --id localclaw-auth
        ;;
    *)
