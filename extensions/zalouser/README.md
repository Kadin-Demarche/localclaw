...
LocalClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
localclaw channels login --channel zalouser
...
```bash
localclaw message send --channel zalouser --target <threadId> --message "Hello from LocalClaw!"
```
...
```bash
localclaw channels login --channel zalouser              # Login via QR
localclaw channels login --channel zalouser --account work
localclaw channels status --probe
localclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
localclaw directory self --channel zalouser
localclaw directory peers list --channel zalouser --query "name"
localclaw directory groups list --channel zalouser --query "work"
localclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
localclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
localclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `localclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
localclaw channels login --channel zalouser --account work
localclaw message send --channel zalouser --account work --target <id> --message "Hello"
