...
`localclaw.invoke --each`) without adding new LocalClaw code per workflow.
...
This extension depends on LocalClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** LocalClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.localclaw/extensions` as a standalone plugin directory.
