...
// NOTE: This extension is intended to be bundled with LocalClaw.
// When running from source (tests/dev), LocalClaw internals live under src/.
...
export function createLlmTaskTool(api: LocalClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via localclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-llm-task-"));
