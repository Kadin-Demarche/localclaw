...
function resolveLocalClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.LOCALCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set LOCALCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveLocalClawRoot(), "dist", "extensionAPI.js");
