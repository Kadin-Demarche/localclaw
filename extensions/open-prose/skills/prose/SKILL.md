...
metadata: { "localclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == LocalClaw `sessions_spawn`
- **File I/O** == LocalClaw `read`/`write`
- **Remote fetch** == LocalClaw `web_fetch` (or `exec` with curl when POST is required)
