import type { PluginRuntime } from "localclaw/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.LOCALCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "localclaw-nostr-"));
  process.env.LOCALCLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.LOCALCLAW_STATE_DIR?.trim() || env.LOCALCLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".localclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.LOCALCLAW_STATE_DIR = previous;
    }
