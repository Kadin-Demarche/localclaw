...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["localclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["localclaw", "pollId"]) ??
    readNestedString(value, ["localclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "localclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "localclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          localclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "localclaw poll vote",
          displayText: "Vote recorded",
          value: { localclawPollId: pollId, pollId },
        },
      },
    ],
  };
