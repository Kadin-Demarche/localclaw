...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: LOCALCLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: LocalClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var LOCALCLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.LOCALCLAW_TWITCH_ACCESS_TOKEN?.trim();
