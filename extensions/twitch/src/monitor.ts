...
import type { ReplyPayload, LocalClawConfig } from "localclaw/plugin-sdk";
import { createReplyPrefixOptions } from "localclaw/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // LocalClawConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as LocalClawConfig;
