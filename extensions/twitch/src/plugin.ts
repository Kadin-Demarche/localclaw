/**
 * Twitch channel plugin for LocalClaw.
 *
...
import { buildChannelConfigSchema } from "localclaw/plugin-sdk";
import type {
 ...
 * for LocalClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: LocalClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: LocalClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: LocalClawConfig): boolean => {
