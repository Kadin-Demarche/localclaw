...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as LocalClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as LocalClawConfig);
