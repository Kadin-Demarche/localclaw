...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = LocalClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = LocalClawCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = LocalClawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = LocalClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = LocalClawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__localclaw__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: LocalClawLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for localclaw:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case LocalClawLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case LocalClawCanvasCommand.present.rawValue,
                 LocalClawCanvasCommand.hide.rawValue,
                 LocalClawCanvasCommand.navigate.rawValue,
                 LocalClawCanvasCommand.evalJS.rawValue,
                 LocalClawCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case LocalClawCanvasA2UICommand.reset.rawValue,
                 LocalClawCanvasA2UICommand.push.rawValue,
                 LocalClawCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case LocalClawCameraCommand.list.rawValue,
                 LocalClawCameraCommand.snap.rawValue,
                 LocalClawCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case LocalClawScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: LocalClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LocalClawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(LocalClawLocationGetParams.self, from: req.paramsJSON)) ??
            LocalClawLocationGetParams()
        ...
        let payload = LocalClawLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LocalClawCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(LocalClawCanvasPresentParams.self, from: req.paramsJSON)) ??
                LocalClawCanvasPresentParams()
        ...
        case LocalClawCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LocalClawCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(LocalClawCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LocalClawCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(LocalClawCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LocalClawCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(LocalClawCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case LocalClawCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing localclawA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LocalClawCanvasA2UICommand.push.rawValue, LocalClawCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == LocalClawCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(LocalClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try LocalClawCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(LocalClawCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try LocalClawCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.localclawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case LocalClawCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LocalClawCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(LocalClawCameraSnapParams.self, from: req.paramsJSON)) ??
                LocalClawCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LocalClawCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(LocalClawCameraClipParams.self, from: req.paramsJSON)) ??
                LocalClawCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(LocalClawScreenRecordParams.self, from: req.paramsJSON)) ??
            LocalClawScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> LocalClawLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return LocalClawLocationMode(rawValue: raw) ?? .off
    }
