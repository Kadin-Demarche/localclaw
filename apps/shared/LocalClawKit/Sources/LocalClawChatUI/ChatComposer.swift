...
@MainActor
struct LocalClawChatComposer: View {
    @Bindable var viewModel: LocalClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(LocalClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(LocalClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \LocalClawPendingAttachment.id)
                { (att: LocalClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            LocalClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(LocalClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(LocalClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(LocalClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(LocalClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
