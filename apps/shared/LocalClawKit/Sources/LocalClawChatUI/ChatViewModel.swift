...
private let chatUILogger = Logger(subsystem: "ai.localclaw", category: "LocalClawChatUI")

@MainActor
@Observable
public final class LocalClawChatViewModel {
    public private(set) var messages: [LocalClawChatMessage] = []
    ...
    public var attachments: [LocalClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [LocalClawChatPendingToolCall] = []
    public private(set) var sessions: [LocalClawChatSessionEntry] = []
    private let transport: any LocalClawChatTransport
    ...
    private var pendingToolCallsById: [String: LocalClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any LocalClawChatTransport) {
    ...
    public var sessionChoices: [LocalClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [LocalClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [LocalClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: LocalClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [LocalClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LocalClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [LocalClawChatMessage]) -> [LocalClawChatMessage] {
        var result: [LocalClawChatMessage] = []
    ...
    private static func dedupeKey(for message: LocalClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [LocalClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> LocalClawChatAttachmentPayload in
            LocalClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> LocalClawChatSessionEntry {
        LocalClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: LocalClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: LocalClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: LocalClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LocalClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> LocalClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
