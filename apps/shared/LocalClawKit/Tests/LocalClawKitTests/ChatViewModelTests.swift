...
import Foundation
import Testing
@testable import LocalClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, LocalClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [LocalClawChatHistoryPayload]
    private let sessionsResponses: [LocalClawChatSessionsListResponse]

    private let stream: AsyncStream<LocalClawChatTransportEvent>
    private let continuation: AsyncStream<LocalClawChatTransportEvent>.Continuation

    init(
        historyResponses: [LocalClawChatHistoryPayload],
        sessionsResponses: [LocalClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<LocalClawChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<LocalClawChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> LocalClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? LocalClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [LocalClawChatAttachmentPayload]) async throws -> LocalClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return LocalClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> LocalClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? LocalClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: LocalClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = LocalClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = LocalClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { LocalClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = LocalClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = LocalClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = LocalClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { LocalClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
