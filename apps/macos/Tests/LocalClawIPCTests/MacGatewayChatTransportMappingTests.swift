...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: LocalClawProtocol.AnyCodable(["ok": LocalClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: LocalClawProtocol.AnyCodable(["ok": LocalClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = LocalClawProtocol.AnyCodable([
            "runId": LocalClawProtocol.AnyCodable("run-1"),
            "sessionKey": LocalClawProtocol.AnyCodable("main"),
            "state": LocalClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: LocalClawProtocol.AnyCodable(["a": LocalClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
