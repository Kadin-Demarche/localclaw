...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: LocalClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LocalClawProtocol.AnyCodable] = [
            "presence": .init([LocalClawProtocol.AnyCodable(entry)]),
        ]
        let payload = LocalClawProtocol.AnyCodable(payloadMap)
