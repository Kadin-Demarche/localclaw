...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.localclaw", category: "gateway-endpoint")
    ...
    {
        let raw = env["LOCALCLAW_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "LOCALCLAW_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["LOCALCLAW_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
               ...
               configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "LOCALCLAW_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.localclaw", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "localclaw.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: LocalClawConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: LocalClawConfigFile.loadDict())
    ...
    {
        if let envBind = env["LOCALCLAW_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["LOCALCLAW_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
