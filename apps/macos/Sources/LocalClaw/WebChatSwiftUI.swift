...
private let webChatSwiftLogger = Logger(subsystem: "ai.localclaw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: LocalClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> LocalClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(LocalClawChatSessionsListResponse.self, from: data)
    }
    ...
    func sendMessage(
        ...
        attachments: [LocalClawChatAttachmentPayload]) async throws -> LocalClawChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> LocalClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                LocalClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    LocalClawChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    LocalClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<LocalClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any LocalClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = LocalClawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: LocalClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<LocalClawChatView>) -> NSViewController
    {
