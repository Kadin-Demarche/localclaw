...
private let deepLinkLogger = Logger(subsystem: "ai.localclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "LocalClaw is paused", message: "Unpause LocalClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run LocalClaw agent?", message: body) else { return }
        }
