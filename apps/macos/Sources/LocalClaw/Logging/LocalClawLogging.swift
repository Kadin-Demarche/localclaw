...
enum LocalClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = LocalClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = LocalClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        LocalClawLogging.bootstrapIfNeeded()
        let label = LocalClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct LocalClawOSLogHandler: LogHandler {
...
struct LocalClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = LocalClawLogging.parseLabel(self.label)
