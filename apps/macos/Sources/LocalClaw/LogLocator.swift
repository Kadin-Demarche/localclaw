...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LOCALCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/localclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("localclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("localclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/localclaw/ (rolling or stdout), or nil if none exist.
