...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.localclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: LocalClawPeekabooBridgeServices?
    private static var localclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("LocalClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = LocalClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.localclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.localclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class LocalClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.localclaw.peekaboo")
