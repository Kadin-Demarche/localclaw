...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (LocalClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: LocalClawOAuthStore.AnthropicOAuthStatus = LocalClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.localclaw", category: "anthropic-oauth")
...
enum LocalClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let localclawOAuthDirEnv = "LOCALCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "LocalClaw OAuth token file not found"
            case .unreadableFile: "LocalClaw OAuth token file not readable"
            case .invalidJSON: "LocalClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in LocalClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "LocalClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.localclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".localclaw", isDirectory: true)
