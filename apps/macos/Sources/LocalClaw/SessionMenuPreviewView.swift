...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.localclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> LocalClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: LocalClawSessionPreviewEntry,
    ...
    private static func cache(payload: LocalClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: LocalClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [LocalClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [LocalClawKit.AnyCodable]) -> [LocalClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(LocalClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: LocalClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: LocalClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: LocalClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: LocalClawChatMessage) -> String? {
