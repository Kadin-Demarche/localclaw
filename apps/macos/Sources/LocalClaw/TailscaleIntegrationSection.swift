...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.localclaw/localclaw.json. Prefer LOCALCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.localclaw/localclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.localclaw/localclaw.json. Restart the gateway to apply."
        }
