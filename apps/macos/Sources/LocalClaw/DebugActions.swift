...
enum DebugActions {
    private static let verboseDefaultsKey = "localclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "localclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = LocalClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "LocalClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = LocalClawPaths.configURL
        guard
