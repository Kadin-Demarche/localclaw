...
enum CommandResolver {
    private static let projectRootDefaultsKey = "localclaw.gatewayProjectRootPath"
    private static let helperName = "localclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let localclawEntry = root.appendingPathComponent("localclaw.mjs").path
        if FileManager().isReadableFile(atPath: localclawEntry) { return localclawEntry }
        let binEntry = root.appendingPathComponent("bin/localclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/localclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let localclawPaths = self.localclawManagedPaths(home: home)
        if !localclawPaths.isEmpty {
            extras.insert(contentsOf: localclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + localclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func localclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".localclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func localclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLocalClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("localclaw.mjs").path,
            root.appendingPathComponent("bin/localclaw.js").path,
        ]
    ...
    static func hasAnyLocalClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.localclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func localclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let localclawPath = self.projectLocalClawExecutable(projectRoot: root) {
                return [localclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "localclaw", subcommand] + extraArgs
            }
            if let localclawPath = self.localclawExecutable(searchPaths: searchPaths) {
                return [localclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            localclaw entrypoint missing (looked for dist/index.js or localclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real localclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/localclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v localclaw >/dev/null 2>&1; then
          CLI="$(command -v localclaw)"
          localclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/localclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/localclaw.mjs"
            node "$PRJ/localclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/localclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/localclaw.js"
            node "$PRJ/bin/localclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent localclaw"
          pnpm --silent localclaw \(quotedArgs);
        else
          echo "localclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? LocalClawConfigFile.loadDict()
