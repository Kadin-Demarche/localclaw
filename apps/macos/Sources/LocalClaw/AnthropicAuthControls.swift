...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: LocalClawOAuthStore.AnthropicOAuthStatus = LocalClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: LocalClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = LocalClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = LocalClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try LocalClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. LocalClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: LocalClawOAuthStore.AnthropicOAuthStatus,
