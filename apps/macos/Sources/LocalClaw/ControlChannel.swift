...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LocalClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.localclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LocalClawKit.AnyCodable]()) {
                $0[$1.key] = LocalClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LocalClawProtocol.AnyCodable?) -> [String: LocalClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LocalClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LocalClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LocalClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("localclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("localclaw.control.agent")
}
