package ai.localclaw.android.voice
...
class VoiceWakeCommandExtractorTest {
  ...
  fun extractsCommandAfterTriggerWord() {
    val res = VoiceWakeCommandExtractor.extractCommand("Claude take a photo", listOf("localclaw", "claude"))
    assertEquals("take a photo", res)
  }

  @Test
  fun extractsCommandWithPunctuation() {
    val res = VoiceWakeCommandExtractor.extractCommand("hey localclaw, what's the weather?", listOf("localclaw"))
